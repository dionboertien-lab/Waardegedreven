...
const FEEDS = [
  ...
  { name: 'Anthropic (nieuws, mogelijk geen RSS)', url: 'https://www.anthropic.com/news', enabled: false }
];
...
async function fetchFeed(url){
  // Proxy via AllOrigins to avoid CORS issues
...
async function loadFeeds(){
  ...
  for (const f of enabled){
    try{
      ...
      console.warn('Feed mislukt', f.name, e);
    }
  }
  ...
  if (!subset.length){
    feedEl.innerHTML = '<p>Geen items gevonden. Controleer je feed-configuratie in <code>assets/js/feed.js</code>.</p>';
    return;
  }
...
function renderCard(item){
  ...
  return `<article class="feed-card">
    ...
    <p><small>${item.source}${date? ' • ' + date : ''}</small></p>
  </article>`;
}
